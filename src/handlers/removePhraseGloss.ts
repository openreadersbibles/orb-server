...
export async function removePhraseGloss(req: Request<GlossIdParams, boolean>, res: Response, userInfo: CognitoUserInfoResponse) {
    ...
    return await ConnectRunDisconnect<boolean>((adapter) => {
        return adapter.deleteGloss(userInfo.username, params.data.gloss_id, 'phrase_gloss');
    });
}